...
@rem Microsoft Store アプリケーションの無効化
cd C:\maintenanceBat\Assets\LGPO
LGPO.exe /m trainingPC.pol /v
echo;
...
@rem 個人用設定 > スタート > "スタート画面にアカウント関連の通知を時々表示する" オフ　/LGPO制御と一部重複している
...
@rem EdgeをInPrivateモードで起動するアイコンをデスクトップに配置する（強制InPrivateを採用中のため不要）
