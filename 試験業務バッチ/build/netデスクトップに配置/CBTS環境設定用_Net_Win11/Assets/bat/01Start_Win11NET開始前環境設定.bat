...
set WALLPAPER=%USERPROFILE%\Desktop\CBTS環境設定用_Net_Win11\Assets\img\Net_ready.jpg
reg add "HKCU\Control Panel\Desktop" /v wallpaper /t REG_SZ /d "%WALLPAPER%" /f
RUNDLL32.EXE user32.dll,UpdatePerUserSystemParameters
echo;

rem バッチを最新化（開始前バッチ処理では終了後バッチをコピーする）
xcopy /y Y:\PC設定用\設定バッチ配布元\02End_Win11NET終了後環境設定.bat %USERPROFILE%\Desktop\CBTS環境設定用_Net_Win11\Assets\bat
